...
> Variable 2: depth (lake_limno)

> Variable 3: chlorophlla a (epi_nutr)

> Variable 4: doc (epi_nutr)

> Variable 5: secchi disk transpancy (epi_nutr)
...
LAGOSL <- left_join(LAGOSlocations, LAGOSdepth, by = "lagoslakeid") 
LAGOStrophic <- left_join(LAGOSnutrient, LAGOSL, by = "lagoslakeid") %>%
        select(lagoslakeid, sampledate, chla, maxdepth, secchi, 
         doc,tn,tp, state, state_name) %>%
    mutate(sampleyear = year(sampledate), 
         samplemonth = month(sampledate), 
         season = as.factor(quarter(sampledate, fiscal_start = 12))) %>%
 drop_na(tn:tp) %>%
 drop_na(secchi:doc) %>%
 drop_na(chla:maxdepth) 
...
```{r}
M <- cor(LAGOStrophic[,3:8])
LAGOStrophic.cor <- corrplot(M, method = "square", type = "lower")
```
...
```{r}
LAGOStrophic.tnlm <- lm(data = LAGOStrophic, tn ~ chla + secchi + state + doc + maxdepth)
summary(LAGOStrophic.tnlm)
LAGOStrophic.tplm <- lm(data = LAGOStrophic, tp ~ chla + secchi + state + doc + maxdepth)
summary(LAGOStrophic.tplm)
```
