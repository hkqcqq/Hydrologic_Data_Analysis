...
dissolved oxygen, nutrients, microbes, odor/ color, invasive species, contaminants
## Session Set Up
...
options(scipen = 100) # bigger than 100 show the exact value
...
LAGOSnutrient <- LAGOSdata$epi_nutr # database only have surface data
...
```{r}
# Join data frames
LAGOSlocations <- left_join(LAGOSlocus, LAGOSstate, by = "state_zoneid") # add column from the right
...
* *Secchi disk transparency*, a measure of water clarity. Pros: simple and cheap. Cons: may yield a high TSI in highly colored lakes and in lakes where particulate matter is comprised of non-algal material #dissolved organic, suspend sediments, shadow
* *Total phosphorus (TP)*, a nutrient essential for growth of primary producers. Assumptions: phosphorus is the limiting nutrient for phytoplankton growth (this assumption often holds only for summer months) # vary with seasons
...
LAGOStrophic <- 
  ...
  drop_na(chla:secchi) #remove entire row that has NA in chla or secchi
...
ggplot(LAGOStrophic, 
  ...
  geom_jitter(alpha = 0.2) + #width of one month
...
LAGOStrophic <- 
  mutate(LAGOStrophic, 
         TSI.chl = round(10*(6 - (2.04 - 0.68*log(chla)/log(2)))), #log is natural log, log10 is based on 10
...
> Violin: distribution, not show the number of data

> Bar: we can know the number of data 
