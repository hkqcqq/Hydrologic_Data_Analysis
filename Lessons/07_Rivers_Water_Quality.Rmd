...
dat <- dat.raw %>%
  ...
  mutate(Site = case_when(Site == "03201300" ~ "Kanawha River",
                          Site == "03206790" ~ "Twelvepole Creek")) #case_when is like if_or function
...
> The PH of Kanawha River is much higher than Twelvepole Creek.


Why might mining be causing the changes you see?

> makes the PH of river rise and specific conductance rise
...
> PH and specific conductance of K River becomes more consistent, but PH of T Creek decreases greatly, and Specific Conductance also decreases
  variablity and level
...
> There is big difference between the pH and specific conductance before regulation and after.

What can you say with data visualization that you can't say with just the statistical tests?

> The regulation is statistically significant. 
...
Manganese.plot.point <- ggplot(Manganese.dat, aes(x = Date, y = Manganese_ugL)) +
  ...
  geom_smooth() +
  geom_vline(xintercept = 1977, color = "grey", alpha = 0.8, lwd = 2, lty = 2) + 
  geom_hline(yintercept = 50, color = "red", alpha = 0.8, lwd = 2, lty = 2) + 
...
> Manganeses data of water in many days are above 0.05 mg/L. 
