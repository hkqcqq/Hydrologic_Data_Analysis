...
* **y**: latitude (required)
* **z**: altitude (optional) contour line, color
...
state.maine <- filter(states, ID %in% 
                          c("maine")) 
LAGOSmaineplot <- ggplot(state.maine) +
  geom_sf(fill = "white")
print(LAGOSmaineplot)
...
Secchiplot.maine <- ggplot() +
  geom_sf(data = state.maine, fill = "white") +
  geom_sf(data = secchi.spatial.maine, aes(color = secchi.mean), 
          alpha = 0.5, size = 1) +
  scale_color_viridis_c() +
  labs(color = "Average Secchi Depth (m)") +
  theme(legend.position = "top")
print(Secchiplot.maine)
```
...
Secchi.Animation <- ggplot() +
  ...
  transition_states(decade, state_length = 1, transition_length = 1) +
  enter_appear() +
  exit_disappear() 
