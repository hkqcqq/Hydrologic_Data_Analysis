...
> aggregation: get the dataset together  

> subsampling: 

> interpolation: filling in gaps
...
* **modified Mann-Kendall**: no seasonality, non-parametric, accounts for temporal autocorrelation, missing data allowed. Function: `mmky` and `mmkh` (package: modifiedmk) #more conservative
...
lake_info(name = "Lake Mendota", state = "Wisconsin") #lots of lakes have same name
...
* **Piecewise constant**: <nearast neigborhood constant>
...
linearinterpolation <- as.data.frame(approx(Mendotadata, n = 500, method = "linear")) #30 days a month
...
Mendotadatats <- linearinterpolation %>%
...
Mendotatimeseries <- ts(Mendotadatats$TP, frequency = 12, 
...
> significant decreasing trend in Octorber and November
