...
LAGOSdata <- load(file = "./Data/Raw/LAGOSdata.rda")
...
secchidepth.lakeareaplot <- 
  ggplot(secchi.spatial.maine, aes(x = area, y = secchi.mean)) +
  geom_point() +
  labs(x = "Lake Area (ha)", y = "Mean Secchi Depth (m)")
print(secchidepth.lakeareaplot)

secchidepth.lakedepthplot <- 
  ggplot(secchi.spatial.maine, aes(x = depth, y = secchi.mean)) +
  geom_point() +
  labs(x = "Lake Depth (m)", y = "Mean Secchi Depth (m)")
print(secchidepth.lakedepthplot)

secchidepth.lakearea <- lm(data = secchi.spatial.maine, secchi.mean ~ area)
summary(secchidepth.lakearea)

secchidepth.lakedepth <- lm(data = secchi.spatial.maine, secchi.mean ~ depth)
...
```{r}
FLlayers <- ggplot() +
  geom_sf(data = HUC6.FL, aes(fill = Name), color = "gray", alpha = 0.5) +
