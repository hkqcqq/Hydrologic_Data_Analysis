...
LAGOStrophic <-
  mutate(LAGOStrophic,
         trophic.class.differ.secchi = 
           ifelse(trophic.class == trophic.class.secchi, "same", "different"),
         trophic.class.differ.tp = 
           ifelse(trophic.class == trophic.class.tp, "same", "different"))
LAGOStrophic %>% count(trophic.class.differ.secchi)
LAGOStrophic %>% count(trophic.class.differ.tp)
```

What proportion of observations do these metrics differ?

>  63.29% of observations differ in trophic states of chlorophyll a concentration and trophic states of secchi disk transparency. 64.54% of observations differ in trophic states of chlorophyll a concentration and trophic states of total phosphorus (TP).
...
LAGOSTPviolin <- ggplot(LAGOSNandP, aes(x = state, y = tp)) +
  geom_violin(aes(fill = state),draw_quantiles = 0.50) 
print(LAGOSTPviolin)
```
